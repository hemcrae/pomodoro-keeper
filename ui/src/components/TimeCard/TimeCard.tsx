...
export const TimeCard: React.FC<EntriesProps> = ({ entries }) => {
  const formattedEntries = entries.reduce(
    (acc: FormattedEntriesObject, entry) => {
              ...
              {formattedEntries.entries &&
                formattedEntries.entries.map((entry) => (
                  <div key={entry.id} className="time-card__entry">
                    <h2 className="time-card__taskName">{entry.name}</h2>
                    <h3 className="time-card__hours">
                      {entry.formattedStartTime} - {entry.formattedEndTime}
                    </h3>
                    <h3 className="time-card__timer">{entry.formattedTotal}</h3>
                  </div>
                ))}
            </div>
          </CardContent>
